(function(){
  ...
  function addTime(seconds){
    seconds=parseInt(seconds,10)||0;if(seconds<=0)return;
    var log=read();log.activeSeconds=(log.activeSeconds||0)+seconds;log.onlineSeconds=(log.onlineSeconds||0)+seconds;write(log);
  }
  function summary(){
    var log=read(), mods=Object.keys(log.modules||{}).map(function(k){var m=log.modules[k];return {id:k,label:m.label||LABELS[k]||k,points:m.points||0,count:m.count||0,unit:m.unit||'分'};}).filter(function(m){return m.points>0||m.count>0;});
    var total=mods.reduce(function(a,m){return a+(m.points||0);},0);
    var minutes=Math.max(0,Math.round((log.activeSeconds||0)/60));
    var days=Math.max(.5,Math.min(9.9,(minutes/30)+(total/600)));
    return {user:name(),date:displayDate(),minutes:minutes,totalPoints:total,modules:mods,days:Math.round(days*10)/10,raw:log};
  }
  ...
  var lastActive=Date.now(), lastTick=Date.now();
  ...
  if(current){
    setInterval(function(){
      var now=Date.now(), delta=Math.min(TICK,now-lastTick);lastTick=now;
      if(document.hidden)return;
      if(current.passive||now-lastActive<=IDLE_LIMIT)addTime(Math.round(delta/1000));
    },TICK);
    document.addEventListener('visibilitychange',function(){lastTick=Date.now();if(!document.hidden)lastActive=Date.now();});
  }
